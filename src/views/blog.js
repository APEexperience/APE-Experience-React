import React from 'react'
import { Link } from 'react-router-dom'

import Script from 'dangerous-html/react'
import { Helmet } from 'react-helmet'

import './blog.css'

const Blog = (props) => {
  return (
    <div className="blog-container">
      <Helmet>
        <title>Blog - APE Experience</title>
        <meta
          name="description"
          content="Tecnología, programación, diseño web, tendencias, informática, y mucho más en APE Blog."
        />
        <meta property="og:title" content="Blog - APE Experience" />
        <meta
          property="og:description"
          content="Tecnología, programación, diseño web, tendencias, informática, y mucho más en APE Blog."
        />
      </Helmet>
      <div className="blog-header">
        <header data-thq="thq-navbar" className="blog-navbar">
          <Link to="/" className="blog-navlink">
            <img
              alt="APE Experience"
              src="/Material web/logo-optimizado-600h.png"
              className="blog-image"
            />
          </Link>
          <div
            data-thq="thq-navbar-nav"
            data-role="Nav"
            className="blog-desktop-menu"
          >
            <nav
              data-thq="thq-navbar-nav-links"
              data-role="Nav"
              className="blog-nav"
            >
              <Link to="/" className="blog-link navLink">
                Inicio
              </Link>
              <Link to="/sobre-mi" className="blog-link1 navLink">
                Sobre mi
              </Link>
              <Link to="/servicios" className="blog-link2 navLink">
                Servicios
              </Link>
              <Link to="/proyectos" className="blog-link3 navLink">
                Proyectos
              </Link>
              <Link to="/blog" className="blog-link4 navLink">
                Blog
              </Link>
              <Link to="/contacto" className="blog-link5 navLink">
                Contacto
              </Link>
            </nav>
            <Link to="/solicitar-servicio" className="blog-register button">
              <span className="blog-text">
                <span>Solicitar </span>
                <br></br>
              </span>
              <svg viewBox="0 0 1024 1024" className="blog-icon">
                <path d="M512 170l342 342-342 342-60-60 238-240h-520v-84h520l-238-240z"></path>
              </svg>
            </Link>
          </div>
          <div data-thq="thq-burger-menu" className="blog-burger-menu">
            <svg viewBox="0 0 1024 1024" className="blog-icon02">
              <path d="M128 554.667h768c23.552 0 42.667-19.115 42.667-42.667s-19.115-42.667-42.667-42.667h-768c-23.552 0-42.667 19.115-42.667 42.667s19.115 42.667 42.667 42.667zM128 298.667h768c23.552 0 42.667-19.115 42.667-42.667s-19.115-42.667-42.667-42.667h-768c-23.552 0-42.667 19.115-42.667 42.667s19.115 42.667 42.667 42.667zM128 810.667h768c23.552 0 42.667-19.115 42.667-42.667s-19.115-42.667-42.667-42.667h-768c-23.552 0-42.667 19.115-42.667 42.667s19.115 42.667 42.667 42.667z"></path>
            </svg>
          </div>
          <div data-thq="thq-mobile-menu" className="blog-mobile-menu">
            <div
              data-thq="thq-mobile-menu-nav"
              data-role="Nav"
              className="blog-nav1"
            >
              <div className="blog-container01">
                <h2 className="blog-text03">OnConf</h2>
                <div data-thq="thq-close-menu" className="blog-menu-close">
                  <svg viewBox="0 0 1024 1024" className="blog-icon04">
                    <path d="M810 274l-238 238 238 238-60 60-238-238-238 238-60-60 238-238-238-238 60-60 238 238 238-238z"></path>
                  </svg>
                </div>
              </div>
              <nav
                data-thq="thq-mobile-menu-nav-links"
                data-role="Nav"
                className="blog-nav2"
              >
                <span className="navLink blog-text04">About</span>
                <span className="navLink blog-text05">Features</span>
                <span className="navLink blog-text06">Pricing</span>
                <span className="navLink blog-text07">Team</span>
                <span className="navLink blog-text08">Blog</span>
              </nav>
              <div className="blog-button-container">
                <button className="blog-login button">Login</button>
                <button className="button blog-register1">Register</button>
              </div>
              <div className="blog-icon-group">
                <div className="blog-icons">
                  <svg
                    viewBox="0 0 950.8571428571428 1024"
                    className="blog-icon06"
                  >
                    <path d="M925.714 233.143c-25.143 36.571-56.571 69.143-92.571 95.429 0.571 8 0.571 16 0.571 24 0 244-185.714 525.143-525.143 525.143-104.571 0-201.714-30.286-283.429-82.857 14.857 1.714 29.143 2.286 44.571 2.286 86.286 0 165.714-29.143 229.143-78.857-81.143-1.714-149.143-54.857-172.571-128 11.429 1.714 22.857 2.857 34.857 2.857 16.571 0 33.143-2.286 48.571-6.286-84.571-17.143-148-91.429-148-181.143v-2.286c24.571 13.714 53.143 22.286 83.429 23.429-49.714-33.143-82.286-89.714-82.286-153.714 0-34.286 9.143-65.714 25.143-93.143 90.857 112 227.429 185.143 380.571 193.143-2.857-13.714-4.571-28-4.571-42.286 0-101.714 82.286-184.571 184.571-184.571 53.143 0 101.143 22.286 134.857 58.286 41.714-8 81.714-23.429 117.143-44.571-13.714 42.857-42.857 78.857-81.143 101.714 37.143-4 73.143-14.286 106.286-28.571z"></path>
                  </svg>
                  <svg
                    viewBox="0 0 877.7142857142857 1024"
                    className="blog-icon08"
                  >
                    <path d="M585.143 512c0-80.571-65.714-146.286-146.286-146.286s-146.286 65.714-146.286 146.286 65.714 146.286 146.286 146.286 146.286-65.714 146.286-146.286zM664 512c0 124.571-100.571 225.143-225.143 225.143s-225.143-100.571-225.143-225.143 100.571-225.143 225.143-225.143 225.143 100.571 225.143 225.143zM725.714 277.714c0 29.143-23.429 52.571-52.571 52.571s-52.571-23.429-52.571-52.571 23.429-52.571 52.571-52.571 52.571 23.429 52.571 52.571zM438.857 152c-64 0-201.143-5.143-258.857 17.714-20 8-34.857 17.714-50.286 33.143s-25.143 30.286-33.143 50.286c-22.857 57.714-17.714 194.857-17.714 258.857s-5.143 201.143 17.714 258.857c8 20 17.714 34.857 33.143 50.286s30.286 25.143 50.286 33.143c57.714 22.857 194.857 17.714 258.857 17.714s201.143 5.143 258.857-17.714c20-8 34.857-17.714 50.286-33.143s25.143-30.286 33.143-50.286c22.857-57.714 17.714-194.857 17.714-258.857s5.143-201.143-17.714-258.857c-8-20-17.714-34.857-33.143-50.286s-30.286-25.143-50.286-33.143c-57.714-22.857-194.857-17.714-258.857-17.714zM877.714 512c0 60.571 0.571 120.571-2.857 181.143-3.429 70.286-19.429 132.571-70.857 184s-113.714 67.429-184 70.857c-60.571 3.429-120.571 2.857-181.143 2.857s-120.571 0.571-181.143-2.857c-70.286-3.429-132.571-19.429-184-70.857s-67.429-113.714-70.857-184c-3.429-60.571-2.857-120.571-2.857-181.143s-0.571-120.571 2.857-181.143c3.429-70.286 19.429-132.571 70.857-184s113.714-67.429 184-70.857c60.571-3.429 120.571-2.857 181.143-2.857s120.571-0.571 181.143 2.857c70.286 3.429 132.571 19.429 184 70.857s67.429 113.714 70.857 184c3.429 60.571 2.857 120.571 2.857 181.143z"></path>
                  </svg>
                  <svg
                    viewBox="0 0 602.2582857142856 1024"
                    className="blog-icon10"
                  >
                    <path d="M548 6.857v150.857h-89.714c-70.286 0-83.429 33.714-83.429 82.286v108h167.429l-22.286 169.143h-145.143v433.714h-174.857v-433.714h-145.714v-169.143h145.714v-124.571c0-144.571 88.571-223.429 217.714-223.429 61.714 0 114.857 4.571 130.286 6.857z"></path>
                  </svg>
                </div>
              </div>
            </div>
            <img
              alt="image"
              src="/quote-background.svg"
              className="blog-background"
            />
          </div>
        </header>
      </div>
      <div className="blog-container02"></div>
      <div className="blog-container03">
        <div className="blog-container04">
          <h1>Blog</h1>
        </div>
      </div>
      <div className="blog-container05">
        <div className="blog-container06">
          <h1 className="blog-text10">
            Personaliza tu servicio ajustado a lo que necesitas para tu negocio
          </h1>
        </div>
        <button type="button" className="blog-button button">
          Enviar ahora
        </button>
      </div>
      <div>
        <div className="blog-container08">
          <Script
            html={`<script>
    ScrollReveal().reveal('.speaker');
    ScrollReveal().reveal('.image-notes');
</script>`}
          ></Script>
        </div>
      </div>
      <div>
        <div className="blog-container10">
          <Script
            html={`<style>
#scrollbar::-webkit-scrollbar-track
{
	-webkit-box-shadow: inset 0 0 6px rgba(0,0,0,0.3);
	border-radius: 10px;
	background-color: #F5F5F5;
}

#scrollbar::-webkit-scrollbar
{
	height: 12px;
	background-color: #F5F5F5;
	border-radius: 10px;
}

#scrollbar::-webkit-scrollbar-thumb
{
	border-radius: 10px;
	-webkit-box-shadow: inset 0 0 6px rgba(0,0,0,.3);
	background-color: #555;
}
</style>`}
          ></Script>
        </div>
      </div>
      <footer className="blog-footer">
        <div className="blog-content">
          <div className="blog-details">
            <Link to="/" className="blog-navlink1">
              <img
                alt="APE Experience Pié de Página Logo Negro"
                src="/Material web/logo-optimizado-600h.png"
                className="blog-image1"
              />
            </Link>
            <p className="blog-description">
              Soluciones a medida para crear tu sitio web profesional, atractivo
              y funcional con la tecnología más actualizada totalmente a tu
              alcance.
            </p>
          </div>
        </div>
        <div className="blog-content1">
          <div className="blog-details1">
            <span className="blog-text11">Navegación</span>
            <Link to="/" className="blog-navlink2 a.link">
              <span className="blog-text12">Inicio</span>
              <br></br>
            </Link>
            <Link to="/sobre-mi" className="blog-navlink3 a.link">
              Sobre mi
            </Link>
            <Link to="/servicios" className="blog-navlink4 a.link">
              Servicios
            </Link>
            <Link to="/proyectos" className="blog-navlink5 a.link">
              Proyectos
            </Link>
            <Link to="/blog" className="blog-navlink6 a.link">
              Blog
            </Link>
            <Link to="/contacto" className="blog-navlink7 a.link">
              Contacto
            </Link>
          </div>
          <div className="blog-socials">
            <a
              href="https://instagram.com/ape_experience/"
              target="_blank"
              rel="noreferrer noopener"
              className="blog-link6"
            >
              <img
                alt="Instagram APE Experience"
                src="/instagram.svg"
                className="blog-image2 social"
              />
            </a>
            <a
              href="https://facebook.com/apeexperienceve/"
              target="_blank"
              rel="noreferrer noopener"
              className="blog-link7"
            >
              <svg
                viewBox="0 0 602.2582857142856 1024"
                className="blog-icon12 social"
              >
                <path d="M548 6.857v150.857h-89.714c-70.286 0-83.429 33.714-83.429 82.286v108h167.429l-22.286 169.143h-145.143v433.714h-174.857v-433.714h-145.714v-169.143h145.714v-124.571c0-144.571 88.571-223.429 217.714-223.429 61.714 0 114.857 4.571 130.286 6.857z"></path>
              </svg>
            </a>
            <a
              href="https://threads.com/ape_experience/"
              target="_blank"
              rel="noreferrer noopener"
              className="blog-link8"
            >
              <img
                alt="Threads APE Experience"
                src="/Material web/threads-600h.png"
                className="blog-image3 social"
              />
            </a>
          </div>
          <span className="blog-copyright">
            Política de Privacidad — Términos y Condiciones — Cookies © 2024
            Derechos reservados por APE Experience.
          </span>
        </div>
      </footer>
    </div>
  )
}

export default Blog
